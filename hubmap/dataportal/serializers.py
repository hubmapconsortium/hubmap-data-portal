...
class ProteinSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        expandable_fields = {
            'genes': (GeneSerializer, {'hugo_symbol', 'entrez_id', 'ensembl_id'})}
        model = Protein
        depth = 3
...
class ScRnaSeqStudyBarcodedSerializer(StudySerializer):
    ...
    def create(self, validated_data):
        scrna_seq_barcoded = ScRnaSeqStudyBarcoded.objects.create(
            **validated_data)
...
class SpatialTranscriptomicStudySerializer(StudySerializer):
    ...
    def create(self, validated_data):
        spatial_transcrptomic = SpatialTranscriptomicStudy.objects.create(
            **validated_data)
...
class MassCytometryStudySerializer(StudySerializer):
    ...
    class Meta(StudySerializer.Meta):
        ...
        write_only_fields = StudySerializer.Meta.write_only_fields + \
            ('preview_image', )
        expandable_fields = {'proteins': (
            GeneSerializer, {'id', 'name', 'pdb_id', 'gene', })}
        model = MassCytometryStudy

    def create(self, validated_data):
        mass_cytometry_study = MassCytometryStudy.objects.create(
            **validated_data)
