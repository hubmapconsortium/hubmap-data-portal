...
class StudyListView(generics.GenericAPIView):
    ...
    def get(self, request, format=None):
        ...
        response.append({"summary": [{"cell_count": cell_count_summary}, {
...
class StudyDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        serializer = self.get_serializer(
            self.get_object().get_subclass_object())
...
class Tissue_svg_colors(views.APIView):
    ...
    def get(self, request):
        ...
        values = [
            {"tissue": tissue, "color": rgba_float_to_rgb_hex(
                matplotlib.cm.viridis(expr))}
...
def compute_multi_dim_counts(queryset, field_name: str) -> xr.DataArray:
    ...
    for study_obj in queryset:
        sel_dict = {label: getattr(
            study_obj, label).name for label, _ in MULTI_DIM_CLASSES}
...
def serialize_multi_dim_counts(data: xr.DataArray):
    ...
    try:
        ...
        grouped = stacked.groupby(
            'gridcell', squeeze=False).sum().unstack('gridcell')
        df = grouped.to_dataframe('temp')
        df.index.names = dims_to_stack

        columns_list = ["Tissue"]
        columns_list.append(data['institution'].sortby(
            data['institution']).values.tolist())
